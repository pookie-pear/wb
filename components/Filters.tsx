...
const Filters = ({ basePath = '/' }: { basePath?: string }) => {
  ...
  const FilterSection = ({ title, options, current, paramKey }: any) => (
    <div className="mb-8">
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-3">
        {options.map((opt: string | number) => (
          <button
            key={opt}
            onClick={() => updateFilter(paramKey, current === String(opt) ? null : String(opt))}
      ...
      <div className="w-full px-4 md:px-0">
        ...
        <div className={`overflow-hidden transition-all duration-700 ease-in-out ${isOpen ? 'max-h-[1000px] pt-12 opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            <FilterSection title="Fit" options={FITS} current={currentFit} paramKey="fit" />
            <FilterSection title="Wash" options={WASHES} current={currentWash} paramKey="wash" />
            <FilterSection title="Gender" options={GENDERS} current={currentGender} paramKey="gender" />
            <FilterSection title="Size" options={SIZES} current={currentSize} paramKey="size" />
          </div>
        </div>
      </div>
    </div>
